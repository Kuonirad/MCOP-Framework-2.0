...
## 2025-12-19 - [Array reduce vs Simple Loops]
**Learning:** Using `Array.prototype.reduce` for calculating variance or mean is ~5x slower in Node.js (V8) compared to using simple `for` loops, due to closure allocation and function call overhead in tight loops.
**Action:** Replace `reduce`, `map`, and other higher-order array functions with standard `for` loops for performance-critical mathematical operations on large vectors.
