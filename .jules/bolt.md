...
**Learning:** Fusing array filling and normalization loops, combined with inverse multiplication, yielded a ~40% performance improvement in `NovaNeoEncoder`. Separation of these loops caused unnecessary memory traffic (write-read-write vs write-only).
**Action:** Fuse transformation loops where possible and prefer multiplication by inverse for bulk division.
