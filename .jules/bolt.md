...
## 2025-12-19 - [Fused Loops in NovaNeoEncoder]
**Learning:** Fusing the array filling and normalization loops in `NovaNeoEncoder` (for 64k dimensions) yielded a ~36% speedup (1100 to 1500 ops/sec). This avoids a second O(N) pass and associated memory traffic, proving that minimizing array traversals is critical for large vectors in JS.
**Action:** When initializing large arrays with computed values, combine all transformations (scaling, normalization) into the initial filling loop whenever possible.
