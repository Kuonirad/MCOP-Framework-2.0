...
## 2025-12-19 - [Array.prototype.reduce Overhead in Mathematical Operations]
**Learning:** Using `Array.prototype.reduce` (or similar array iteration methods) for mathematical operations on vectors/tensors is significantly slower (~5x) than standard `for` loops in Node.js (V8) due to function call and closure overhead.
**Action:** Always prefer standard `for` loops for performance-critical numerical paths over functional array methods.
