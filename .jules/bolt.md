...
## 2025-12-19 - [Array.prototype.reduce Overhead in V8]
**Learning:** Using `Array.prototype.reduce` (or similar array iteration methods) for mathematical operations on vectors/tensors is significantly slower (~12x) than standard `for` loops in Node.js (V8) due to function call and closure overhead. For performance-critical numerical paths, standard `for` loops and simple multiplication (`x * x` vs `Math.pow(x, 2)`) should be preferred.
**Action:** Replace `reduce` and `Math.pow` with standard `for` loops and direct multiplication in high-frequency calculations like `estimateEntropy`.
