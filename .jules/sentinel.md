...
## 2025-12-19 - Unbounded Synchronous Hashing DoS
**Vulnerability:** The `NovaNeoEncoder` performed synchronous `crypto.createHash` operations on unbounded string inputs, which could allow an attacker to cause memory exhaustion and block the Node.js event loop by passing excessively large strings.
**Learning:** Synchronous operations on unbound inputs in Node.js block the event loop and can cause memory exhaustion DoS attacks.
**Prevention:** It is a security best practice to enforce a `maxInputLength` limit on data before performing expensive synchronous operations.
