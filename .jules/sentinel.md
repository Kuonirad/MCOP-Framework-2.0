...
## 2025-12-19 - Synchronous Crypto Memory Exhaustion DoS
**Vulnerability:** Unbounded synchronous crypto operations in Node.js (specifically `crypto.createHash` in `NovaNeoEncoder`).
**Learning:** Feeding excessively large inputs (e.g., arbitrarily large strings) to synchronous cryptographic functions blocks the Node.js event loop and consumes memory proportionately. This presents a severe Denial of Service (DoS) attack vector, even for relatively standard hashing tasks like SHA-256.
**Prevention:** Always enforce a configurable maximum input length (e.g., `maxInputLength`) before processing data through synchronous, CPU-intensive, or memory-heavy operations in single-threaded environments like Node.js.
