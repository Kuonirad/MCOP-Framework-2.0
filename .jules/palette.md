...
## 2025-12-19 - Decorative Icon Accessibility
**Learning:** Decorative icons inside buttons or links that already contain descriptive text must have the `aria-hidden` attribute applied AND their `alt` attribute explicitly set to an empty string (`alt=""`). Leaving a descriptive `alt` text while using `aria-hidden` creates conflicting instructions for screen readers.
**Action:** Always ensure decorative icons in links have both `aria-hidden` and `alt=""`.
