...
- **Node.js 22.23.1**, downloaded from the official Node.js distribution
  (`nodejs.org/dist`) and verified during the build against separate
  **compile-time SHA-256 archive and executable pins** in
  `scripts/desktop/prepare-node-runtime.mjs`. Archive digests are sourced from
  Node's published `SHASUMS256.txt`; executable digests were computed from the
  extracted files after their archives passed that upstream check. Its
  complete upstream `LICENSE` file is installed under the desktop application
  resources.
