...
export class NovaNeoEncoder {
  ...
  estimateEntropy(tensor: ContextTensor): number {
    ...
    // Optimization: Use standard for loops and simple multiplication instead
    // of Array.prototype.reduce and Math.pow for significant performance gains
    // in V8 (Node.js). Reduces overhead of closures and function calls.
    let sum = 0;
    for (let i = 0; i < len; i++) {
    ...
    for (let i = 0; i < len; i++) {
      ...
      varianceSum += diff * diff; // diff * diff is faster than Math.pow(diff, 2)
    }
