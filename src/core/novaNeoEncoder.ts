...
export class NovaNeoEncoder {
  ...
  estimateEntropy(tensor: ContextTensor): number {
    ...
    // Optimization: Use standard for loops and direct multiplication
    // instead of Array.prototype.reduce and Math.pow. This avoids closure
    // allocation and function call overhead, yielding significant speedups
    // in V8 for numerical calculations.
    let sum = 0;
    for (let i = 0; i < len; i++) {
