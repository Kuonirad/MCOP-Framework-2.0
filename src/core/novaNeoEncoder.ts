...
export class NovaNeoEncoder {
  ...
  estimateEntropy(tensor: ContextTensor): number {
    // Optimized single-pass variance calculation: E[X^2] - (E[X])^2
    ...
    // Use a standard for loop for performance (~12x faster than reduce in V8)
    for (let i = 0; i < len; i++) {
      const abs = Math.abs(tensor[i]);
      sumAbs += abs;
      sumSq += abs * abs;
    }
    ...
    // Clamp to 0 to handle potential floating-point instability where result is slightly negative
