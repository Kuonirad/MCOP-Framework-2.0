...
export class NovaNeoEncoder {
  ...
  estimateEntropy(tensor: ContextTensor): number {
    ...
    // Optimization: Use standard loops instead of Array.prototype.reduce
    // for mathematical operations on vectors to avoid ~12x function call overhead in V8
    let sum = 0;
    for (let i = 0; i < len; i++) {
    ...
    for (let i = 0; i < len; i++) {
      ...
      varianceSum += diff * diff; // Optimization: diff * diff is faster than Math.pow(diff, 2)
    }
