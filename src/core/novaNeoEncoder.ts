...
export class NovaNeoEncoder {
  ...
  constructor(config: NovaNeoConfig) {
    ...
    // Security: Enforce a maximum input length to prevent memory exhaustion DoS
    // from large inputs passed to synchronous crypto operations. Default: 10MB
    this.maxInputLength = config.maxInputLength ?? 10 * 1024 * 1024;
  }

  encode(text: string): ContextTensor {
    if (text.length > this.maxInputLength) {
      throw new Error(`Input length (${text.length}) exceeds maximum allowed length (${this.maxInputLength})`);
    }
