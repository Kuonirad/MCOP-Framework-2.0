...
export class NovaNeoEncoder {
  ...
  estimateEntropy(tensor: ContextTensor): number {
    ...
    // using standard for loops instead of Array.prototype.reduce to avoid closure overhead
    ...
    let entropy = variance;
    if (entropy > 1) entropy = 1;
    return entropy < this.entropyFloor ? this.entropyFloor : entropy;
  }
}
