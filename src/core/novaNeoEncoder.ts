...
export class NovaNeoEncoder {
  ...
  encode(text: string): ContextTensor {
    ...
    // Optimization 4: Fused filling and normalization loop
    // This avoids a second pass over the array for normalization and uses multiplication instead of division
    if (this.normalize) {
      ...
      // Standard filling loop without normalization overhead
      if (hashLen === 32) {
        for (let i = 0; i < this.dimensions; i++) {
