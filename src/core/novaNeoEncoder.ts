...
export class NovaNeoEncoder {
  ...
  constructor(config: NovaNeoConfig) {
    ...
    this.maxInputLength = config.maxInputLength ?? 10 * 1024 * 1024; // Default: 10MB
  }

  encode(text: string): ContextTensor {
    if (text.length > this.maxInputLength) {
      throw new Error(
        `Input text length ${text.length} exceeds maximum allowed length of ${this.maxInputLength}`
      );
    }
