...
export class NovaNeoEncoder {
  ...
  estimateEntropy(tensor: ContextTensor): number {
    ...
    // Optimization: Use standard for loops and diff * diff instead of reduce and Math.pow
    // to avoid function call/closure overhead and reduce function allocation overhead.
    let sum = 0;
    for (let i = 0; i < len; i++) {
