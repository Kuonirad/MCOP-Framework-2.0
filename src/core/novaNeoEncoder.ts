...
export class NovaNeoEncoder {
  ...
  estimateEntropy(tensor: ContextTensor): number {
    ...
    // Optimization: Explicit loops instead of Array.prototype.reduce
    // Math.abs is utilized directly due to better V8 optimizations
    let sumAbs = 0;
    for (let i = 0; i < len; i++) {
    ...
    // Optimization: diff * diff instead of Math.pow(diff, 2)
    let sumDiffSq = 0;
    for (let i = 0; i < len; i++) {
