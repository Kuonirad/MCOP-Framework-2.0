...
export class NovaNeoEncoder {
  ...
  encode(text: string): ContextTensor {
    ...
    if (this.normalize) {
      // Fused normalization: calculate inverse magnitude once and apply during fill
      // Reduces iterations from 2N to N and replaces division with multiplication
      const invNorm = 1 / (Math.sqrt(sumSquares) || 1);
