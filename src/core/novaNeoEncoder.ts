...
export class NovaNeoEncoder {
  ...
  encode(text: string): ContextTensor {
    // Security: Enforce max input length to prevent event loop blocking during synchronous hash
    if (text.length > this.maxInputLength) {
      throw new Error(`Input exceeds maximum length of ${this.maxInputLength} characters`);
    }
