...
export class NovaNeoEncoder {
  ...
  constructor(config: NovaNeoConfig) {
    ...
    this.maxInputLength = config.maxInputLength ?? 10 * 1024 * 1024; // 10MB default limit
  }

  encode(text: string): ContextTensor {
    if (text.length > this.maxInputLength) {
      throw new Error(`Input length exceeds maximum allowed (${this.maxInputLength} characters)`);
    }
