...
export class NovaNeoEncoder {
  ...
  estimateEntropy(tensor: ContextTensor): number {
    ...
    // Optimization: Replaced reduce with a native for-loop to reduce callback allocation overhead
    let sum = 0;
    for (let i = 0; i < len; i++) {
    ...
    // Optimization: Replaced reduce with a native for-loop and Math.pow with direct multiplication
    let varianceSum = 0;
    for (let i = 0; i < len; i++) {
