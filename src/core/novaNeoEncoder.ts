...
export class NovaNeoEncoder {
  ...
  estimateEntropy(tensor: ContextTensor): number {
    ...
    // Optimization: Use native for loops instead of Array.prototype.reduce
    // for performance in hot paths processing large tensors.
    let sumAbs = 0;
    for (let i = 0; i < tensor.length; i++) {
      sumAbs += Math.abs(tensor[i]);
    }
    const mean = sumAbs / tensor.length;
