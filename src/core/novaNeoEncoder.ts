...
export class NovaNeoEncoder {
  ...
  encode(text: string): ContextTensor {
    ...
    // Optimization 4: Optimized filling & normalization loop
    // Fused loop avoids iterating over dimensions twice
    // Multiplication by inverse norm is faster than division
    if (this.normalize) {
      if (hashLen === 32) {
