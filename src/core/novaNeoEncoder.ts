...
export class NovaNeoEncoder {
  ...
  estimateEntropy(tensor: ContextTensor): number {
    ...
    // Optimization: Use standard for loop instead of reduce to avoid
    // closure allocation and function call overhead.
    let sum = 0;
    for (let i = 0; i < len; i++) {
    ...
    for (let i = 0; i < len; i++) {
      ...
      // Optimization: direct multiplication is faster than Math.pow(diff, 2)
      sumSqDiff += diff * diff;
    }
