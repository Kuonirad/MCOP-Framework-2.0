...
export class NovaNeoEncoder {
  ...
  estimateEntropy(tensor: ContextTensor): number {
    ...
    // Optimization: Replaced Array.prototype.reduce and Math.pow with native for loops
    // and direct multiplication to eliminate callback allocation overhead in V8.
    ...
    for (let i = 0; i < len; i++) {
      ...
      sumVariance += diff * diff; // Optimization: Direct multiplication instead of Math.pow
    }
    const variance = sumVariance / len;

    const entropy = variance < 1 ? variance : 1; // Optimization: avoid Math.min
    return entropy > this.entropyFloor ? entropy : this.entropyFloor; // Optimization: avoid Math.max
  }
}
