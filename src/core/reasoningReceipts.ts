...
export function verifyBundle(bundle: ReasoningSessionBundle): {
  ...
  for (let i = 0; i < total; i += 1) {
    ...
    if (!receipt) {
      // Claim is displayed in `bundle.claims` but the published root never
      // sealed a receipt for this position — must be treated as forged.
      results.push({ leafIndex: i, valid: false, reason: 'unsealed-claim' });
      continue;
    }
    if (i >= claims.length) {
      // Mirror case: a receipt with no displayed claim. The receipt itself
      // may be cryptographically intact, but the bundle's claim list is
      // truncated, so a downstream consumer that reads `bundle.claims`
      // would render a session inconsistent with the sealed root.
    ...
    if (leafEntryForClaim(claims[i]) !== receipt.leafEntry) {
      // The receipt's own sealed claim is intact (verifyReceipt passed), but
      // the bundle's claim list disagrees with it — so a renderer iterating
      // `bundle.claims` would show different text than the root commits to.
  ...
  // Cross-check the published `size`. Forging this number alone — without
  // touching the root or receipts — is a one-line attack against any
  // consumer that surfaces the count (the reader page does), and is exactly
  // the silent acceptance the previous loop allowed.
