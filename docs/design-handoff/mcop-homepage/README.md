...
- **Hero row** (4 glass cards in a `1.6fr 1fr 1fr 1fr` grid, collapses to 1fr on mobile):
  ...
  3. Coverage metric: `90.63%`, sub: "jest · documented surface".
...
- **Right — install-prose**: serif h3, two paragraphs about cross-runtime parity, and a chips list (Next.js 15.5, TypeScript 5, Node 22.22.2, pnpm 9.15.0, Python 3.12, Docker, Jest 90.63%, ESLint strict, CodeQL, SBOM, OIDC publish).
...
- `screenshots/` — reference renders of each section, in scroll order:
  - `01-hero.png` — hero h1 tail, lede, CTA card + 3 metric cards (4.4 ms / 90.63% / BUSL 1.1)
